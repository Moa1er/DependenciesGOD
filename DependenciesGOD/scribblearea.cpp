...
//base class is from https://www.newthinktank.com/2018/07/qt-tutorial-5-paint-app/
    ...
    : QWidget(parent)
{
    ...
    qDebug() << parent->maximumHeight();
    qDebug() << parent->maximumWidth();
    ...
    //sends resize event to trigget resizeEvent() function to set image property
...
void ScribbleArea::drawRectangle(int x, int y, int width, int height){
    ...
    drawLineTo(QPoint(x + width, y));
    drawLineTo(QPoint(x + width, y + height));
    drawLineTo(QPoint(x, y+height));
